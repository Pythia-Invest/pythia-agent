...
from . import firds, manifest, mic, schema, sec, us_listed, writer
...
def parse_args(argv: list[str]) -> BuildConfig:
    ...
    parser.add_argument("--no-sec", action="store_true", help="skip US lines (SEC tickers and listed US ETFs)")
...
def run(config: BuildConfig) -> int:
    ...
    listed = us_listed.parse(*us_listed.fetch(downloads, day)) if config.scope.sec else {}
    ...
    inputs = Inputs(config.as_of, config.scope, venues, admissions, transparency, sec_rows, figi.mic_codes(), listed)
