...
describe("declarative visual host", () => {
  ...
  it("embeds only standalone explicit workspace visual links through the actual Markdown renderer", () => {
    const href = "/workspace/working/example.pythia-vega-lite.json";
    ...
    for (const text of [
      ...
      `\`[Explore](${href})\``,
      `\`\`\`md\n[Explore](${href})\n\`\`\``,
      "[External](https://example.test/example.pythia-vega-lite.json)",
      "[Ordinary JSON](/workspace/example.json)",
    ])
      expect(render(text)).not.toContain('data-slot="chat-visual"');
    expect(visualLinkPath(href)).toBe("working/example.pythia-vega-lite.json");
    expect(visualLinkPath("file:///example.pythia-vega-lite.json")).toBeNull();
  });
});
