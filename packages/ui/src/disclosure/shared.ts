...
export const disclosureClasses = {
  ...
  /**
   * Base UI measures the panel; the height variable drives the open/close
   * motion. Panels carry a resting `auto` fallback because Base UI writes the
   * measured value at runtime — without one the declaration is invalid until
   * the first measurement, and a tool reading the compiled stylesheet sees a
   * dangling reference.
   */
  panel:
