...
export type PythiaLockupVariant = "full" | "compact" | "mark";
...
const variants = {
  ...
  /** The pictogram alone, for surfaces that already name Pythia in words. */
  mark: {
    light: brandAssetUrls.markLight,
    dark: brandAssetUrls.markDark,
    width: 256,
    height: 256,
    className: "w-[1em]",
  },
 ...
 *
 * Use `full` for the primary identity, `compact` where horizontal space is
 * constrained, and `mark` for the pictogram alone where the surface already
 * names Pythia in words. Each variant uses a density-bounded raster export of
 * an approved asset, so consumers never reconstruct its wordmark or ship the
 * 2048px pictogram master. It has no interactive states and follows
