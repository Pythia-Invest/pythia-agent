...
import { checkPublicSource } from "../../tooling/check-public-source.mjs";
import {
...
describe("assembled developer readiness", () => {
  ...
  it("keeps private projections and Design Lab outside publication and runtime", () => {
    const fixture = createExactSourceFixture();
    expect(checkPublicSource(fixture.worktreeOne)).toEqual(fixture.source);
    const privateRecord = join(
      fixture.worktreeOne,
      ".private/plans/main/result.md",
    );
    const generatedRole = join(
      fixture.worktreeOne,
      ".codex/agents/reviewer.toml",
    );
    privateFile(privateRecord, "private fixture record\n");
    privateFile(generatedRole, "generated = true\n");
    git(fixture.worktreeOne, ["add", "--force", ".private", ".codex"]);
    expect(() => checkPublicSource(fixture.worktreeOne)).toThrow(
      /private.*entered source/u,
    );

    run(repositoryRoot, "node", ["tooling/check-runtime-closure.mjs"]);
    run(repositoryRoot, "node", ["tooling/check-boundaries.mjs"]);
    const designLab = JSON.parse(
      readFileSync(
        join(repositoryRoot, "apps/design-lab/package.json"),
        "utf8",
      ),
    );
    const desk = JSON.parse(
      readFileSync(join(repositoryRoot, "apps/desk/package.json"), "utf8"),
    );
    expect(designLab.scripts.start).toBeUndefined();
    expect(desk.dependencies["@pythia/ui"]).toBe("workspace:*");
    expect(JSON.stringify(desk)).not.toContain("@pythia/design-lab");
    ...
    run(
      repositoryRoot,
      "pnpm",
      ["--filter", "@pythia/ui", "build"],
      buildEnvironment,
    );
    run(
      repositoryRoot,
      "pnpm",
      ["--filter", "@pythia/design-lab", "build"],
      buildEnvironment,
    );
  }, 60_000);
