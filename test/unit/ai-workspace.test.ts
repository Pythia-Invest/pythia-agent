...
describe("public development-agent workspace", () => {
  test("canonical builder workspace passes its deterministic checker", () => {
    expect(() =>
      execFileSync("node", ["tooling/check-ai-workspace.mjs"], {
        cwd: repository,
        encoding: "utf8",
      }),
    ).not.toThrow();
  ...
  test("planning workflows remain explicit-only in native skill configuration and Claude copies", () => {
    const root = temporaryRoot("explicit-skills");
    execFileSync("node", [path.join(repository, "scripts", "ai-sync.mjs")], {
      env: { ...process.env, PYTHIA_AGENT_CLAUDE_SKILLS_ROOT: root },
    });
    for (const skill of [
      "create-plan",
      "implement-plan",
    ...
    expect(existsSync(path.join(root, "shape-issue", "SKILL.md"))).toBe(false);
    expect(
      existsSync(
        path.join(repository, ".agents", "skills", "shape-issue", "SKILL.md"),
      ),
    ).toBe(false);
  });
