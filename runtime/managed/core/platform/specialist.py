...
from .access import eligible_tools, ContextUnavailable, native_access_scope, fingerprint, owns_tool
from .operations import declare_operation, result_issues as operation_issues
...
def failure(code):
    return {'schema_version': 1, 'outcome': 'error', 'issues': [
        {'code': code, 'severity': 'error', 'message': 'The plugin operation could not be completed.'}]}
...
class ReadSupport:
    ...
    def inspect(self, resource, *, updates=False):
        ...
        return {'scope': None, 'lane': 'ordinary'}

    def result_issues(self, result): return self.issues(result)


def register_read_command(ctx, command_name, tool_name, description, *, cache_seconds=0, cache_overrides=(), schema=None, plugin=None, result_issues=None):
    support = ReadSupport(result_issues) if result_issues else None
    if schema is not None:
        ...
        declare_operation(schema, operation=command_name, plugin=ctx.plugin_id,
                          handler=entry.handler if entry else None, support=support,
                          cache_seconds=cache_seconds if type(cache_seconds) is int else 0,
                          cache_overrides=cache_overrides, updates=True, read_only=True)
    def setup(parser):
    ...
    def command(args):
        ...
        try:
            if tool_name not in eligible_tools() or not owns_tool(tool_name, ctx.plugin_id):
                result = failure('unavailable')
            else:
                if len(args.request.encode()) > 65536:
                    raise ValueError('invalid_request')
                arguments = json.loads(args.request)
                Draft202012Validator(registry.get_schema(tool_name)['parameters']).validate(arguments)
                ...
                if scope and getattr(args, 'reuse_scope', None) == scope:
                    ...
                    if (scope and result.get('outcome') in ('ok', 'partial', 'empty')
                            and not operation_issues(result, support)
                ...
                elif access != native_access_scope() or tool_name not in eligible_tools() or not owns_tool(tool_name, ctx.plugin_id):
                    result = failure('unavailable')
