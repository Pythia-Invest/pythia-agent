...
include_platform=true and /coins/markets. The list has no
...
def identifiers(coin, symbol, pairs):
    ...
    for network, address in pairs:
        values.append({'scheme': 'contract_address', 'value': address, 'network': network, 'authority': 'source_asserted'})
    return values
...
def page(snapshot, arguments):
    ...
    for row in snapshot['rows'][offset:offset + limit]:
        ...
        item = {'provider_ref': native, 'level': 'crypto', 'asset_class': 'crypto', 'status': 'active',
                'name': name, 'symbol': symbol or None, 'identifiers': identifiers(coin, symbol, pairs)}
