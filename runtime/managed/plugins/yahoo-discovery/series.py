...
MODES = ('latest', 'daily', 'adjusted', 'minute', 'five_minute', 'hour')
...
def definition(native, mode, metadata):
    ...
    value = {'schema_version': 1, 'subject': native, 'provider_ref': native,
        ...
        'fields': fields, 'interval': {'kind': 'tick' if mode == 'latest' else 'day' if daily else 'hour' if mode == 'hour' else 'minute', 'count': 5 if mode == 'five_minute' else 1},
        'calendar': None, 'timezone': None if daily else 'UTC', 'session': 'regular', 'time_anchor': 'session_date' if daily else 'instant' if mode == 'latest' else 'interval_start',
